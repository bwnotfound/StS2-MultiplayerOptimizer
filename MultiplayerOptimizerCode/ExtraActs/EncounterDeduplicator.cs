...
public static class EncounterDeduplicator
{
    ...
    public static int DeduplicateAdjacent(IList<EncounterModel> list)
    {
        ...
        while (true)
        {
            ...
            else break; // append 完就 break
        }

        // 写回原 list
        for (int i = 0; i < list.Count; i++) list[i] = result[i];

        // 数实际相邻重复对（用于 caller log）
        int dupPairs = 0;
        for (int i = 1; i < list.Count; i++)
        {
            if (list[i].Id.Entry == list[i - 1].Id.Entry) dupPairs++;
        }
    ...
    public static int DeduplicateMerged(IList<EncounterModel> list1, IList<EncounterModel> list2)
    {
        ...
        for (int i = 0; i < list1.Count; i++) list1[i] = combined[i];
        for (int i = 0; i < list2.Count; i++) list2[i] = combined[list1.Count + i];
