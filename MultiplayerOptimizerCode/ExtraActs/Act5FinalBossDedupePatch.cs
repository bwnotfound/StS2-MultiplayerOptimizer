...
///     玩家走哪条路径到 final boss、上一关 boss 是谁，所以**只能在玩家点击 final boss 节点时**
///     做实时检查
...
public static class Act5FinalBossDedupePatch
{
    ...
    public static void RerollFinalBossIfDuplicate(MapCoord coord)
    {
        ...
        for (var i = 0; i < state.CurrentActIndex; i++) avoidIds.Add(state.Acts[i].BossEncounter.Id.Entry);
        ...
        if (alternatives.Count == 0)
        {
            MainFile.Logger.Warn(
                $"[ExtraActs] Cannot reroll Act5 final boss '{currentBossEntry}': " +
        ...
        MainFile.Logger.Info(
            $"[ExtraActs] Act5 final boss re-rolled: '{currentBossEntry}' -> '{picked!.Id.Entry}' " +
