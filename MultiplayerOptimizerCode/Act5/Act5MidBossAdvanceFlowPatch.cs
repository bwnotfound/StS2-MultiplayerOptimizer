...
public static class Act5MidBossAdvanceFlowPatch
{
    ...
    public static bool Prefix(NRewardsScreen __instance, NButton _)
    {
        ...
        return PatchScope.Run<bool>(nameof(Act5MidBossAdvanceFlowPatch), () =>
        {
            ...
            // 顶端真 boss 节点（第一个真 boss）？放行让原方法触发通关流程
            if (runState.CurrentMapCoord == runState.Map.BossMapPoint.coord) return true;

            // DoubleBoss 第二个 boss 节点（Ascension 10）？也放行——这是真正的通关战
            //
            // base game OnProceedButtonPressed line 314-326 的 DoubleBoss 流程：
            //   - 第一个 boss 胜利 (CurrentMapCoord == BossMapPoint.coord)
            //       → ProceedFromTerminalRewardsScreen → 进入第二个 boss
            //   - 第二个 boss 胜利 (CurrentMapCoord == SecondBossMapPoint.coord)
            //       → _proceedButton.Disable + ActChangeSynchronizer.SetLocalPlayerReady
            //       → 通关结束流程
            //
            // 我们必须放行第二个 boss 战胜利的情况；否则会被误判为"中部伪装 boss"走
            // ProceedFromTerminalRewardsScreen → 永远不结束游戏。
            if (runState.Map.SecondBossMapPoint != null
                && runState.CurrentMapCoord == runState.Map.SecondBossMapPoint.coord) return true;
